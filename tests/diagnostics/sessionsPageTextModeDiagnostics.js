...
describe('Sessions Page Text Mode - Diagnostics', () => {
  ...
  describe('1. Component Import Analysis', () => {
    ...
    it('should verify sendMessage is imported from services', () => {
      const hasImport = sessionsPageContent.includes('sendMessage') &&
  ...
  describe('3. Event Handler Analysis', () => {
    it('should verify handleSendMessage function exists', () => {
      const hasFunctionDef = sessionsPageContent.includes('function handleSendMessage') ||
    ...
    it('should verify handleSendMessage is connected to MessageInput', () => {
      ...
      if (!hasEventBinding) {
        ...
        console.log('MessageInput usage:', 
    ...
    it('should verify handleSendMessage calls sendMessage service', () => {
      const functionBody = sessionsPageContent.match(
        /(?:async )?function handleSendMessage[\s\S]*?{([\s\S]*?)(?=\n  (?:async )?function|\n  onMount|\n<\/script>)/
      )?.[1] || '';
      
      const callsSendMessage = functionBody.includes('sendMessage(') ||
                              functionBody.includes('await sendMessage');
  ...
  describe('4. Store Integration Analysis', () => {
    it('should verify messages store is imported', () => {
      const hasImport = sessionsPageContent.includes('messages') &&
    ...
    it('should verify chatMode store is used', () => {
      ...
      const hasUsage = sessionsPageContent.includes('$chatMode') ||
                      sessionsPageContent.includes('$chatModeStore');
    ...
    it('should verify messages are displayed from store', () => {
      const usesMessagesStore = sessionsPageContent.includes('$messages') ||
                               messageListContent.includes('$messages');
  ...
  describe('5. API Integration Analysis', () => {
    it('should verify messages are saved to session API', () => {
      const hasSaveLogic = sessionsPageContent.includes('/api/sessions/') &&
                          sessionsPageContent.includes('/messages');
      const hasSaveFunction = sessionsPageContent.includes('saveMessageToSession') ||
    ...
    it('should verify messages are loaded from session API', () => {
      const hasLoadLogic = sessionsPageContent.includes('/api/sessions/') &&
                          sessionsPageContent.includes('/messages') &&
    ...
    it('should verify sendMessage service calls /api/chat', () => {
      ...
      const hasFetchCall = chatServicesContent.includes('fetch(') &&
                          chatServicesContent.includes('/api/chat');
  ...
  describe('6. Message Flow Analysis', () => {
    it('should trace complete message flow', () => {
      ...
      const step6 = sessionsPageContent.includes('saveMessageToSession') ||
  ...
  describe('7. Specific Bug Detection', () => {
    it('should check for common issues', () => {
      ...
      if (sessionsPageContent.includes('<MessageInput') && 
      ...
      if (sessionsPageContent.includes('function handleSendMessage') &&
          !sessionsPageContent.includes('async function handleSendMessage')) {
        issues.push('handleSendMessage might not be async');
      }
      
      // Check if sendMessage is awaited
      const handleSendBody = sessionsPageContent.match(
        /function handleSendMessage[\s\S]*?{([\s\S]*?)}/
      )?.[1] || '';
      if (handleSendBody.includes('sendMessage(') && 
      ...
      if (!sessionsPageContent.includes('$messages') && 

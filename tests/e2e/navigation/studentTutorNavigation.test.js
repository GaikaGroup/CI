...
test.describe('Student/Tutor Navigation', () => {
  ...
  test('should switch between Student tabs', async ({ page }) => {
    ...
    await expect(page.locator('button:has-text("Browse Courses")').first()).toHaveClass(
      /border-blue-500/
    );

    // Click My Learning tab
    await page.click('button:has-text("My Learning")');
    await expect(page.locator('button:has-text("My Learning")').first()).toHaveClass(
      /border-blue-500/
    );

    // Click Progress tab
    await page.click('button:has-text("Progress")');
    await expect(page.locator('button:has-text("Progress")').first()).toHaveClass(
      /border-blue-500/
    );
  });
  ...
  test('should switch between Tutor tabs', async ({ page }) => {
    ...
    await expect(page.locator('button:has-text("My Courses")').first()).toHaveClass(
      /border-amber-500/
    );

    // Click Create Course tab
    await page.click('button:has-text("Create Course")');
    await expect(page.locator('button:has-text("Create Course")').first()).toHaveClass(
      /border-amber-500/
    );

    // Click Analytics tab
    await page.click('button:has-text("Analytics")');
    await expect(page.locator('button:has-text("Analytics")').first()).toHaveClass(
      /border-amber-500/
    );

    // Click Students tab
    await page.click('button:has-text("Students")');
    await expect(page.locator('button:has-text("Students")').first()).toHaveClass(
      /border-amber-500/
    );
  });

...
test.describe('Voice Mode UX Enhancement', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      window.AudioContext = class MockAudioContext {
        ...
        createGain() { return { gain: { setValueAtTime: () => {}, linearRampToValueAtTime: () => {}, value: 1 }, connect: () => {} }; }
        createAnalyser() { return { fftSize: 256, getByteFrequencyData: () => {} }; }
        createMediaStreamSource() { return { connect: () => {} }; }
        decodeAudioData() { return Promise.resolve({ duration: 1, getChannelData: () => new Float32Array(1024) }); }
  ...
  test.describe('Audio Stuttering Prevention', () => {
    test('should play audio without stuttering', async ({ page }) => {
      ...
      await page.waitForSelector('[data-testid="speaking-indicator"]', { state: 'hidden', timeout: 10000 });
    ...
    test('should handle waiting phrases smoothly', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="waiting-phrase-indicator"]')).toBeVisible({ timeout: 2000 });
      ...
      await page.waitForSelector('[data-testid="speaking-indicator"]', { state: 'hidden', timeout: 15000 });
  ...
  test.describe('Avatar Mouth Synchronization', () => {
    test('should close mouth properly when speech ends', async ({ page }) => {
      ...
      const mouthDuringSpeech = await page.locator('[data-testid="avatar-mouth"]').getAttribute('data-state');
      expect(mouthDuringSpeech).not.toBe('closed');

      // Wait for speaking to end
      await page.waitForSelector('[data-testid="avatar-speaking"]', { state: 'hidden', timeout: 10000 });

      // Wait a bit for mouth to close
      await page.waitForTimeout(500);

      // Verify mouth is closed after speech
      const mouthAfterSpeech = await page.locator('[data-testid="avatar-mouth"]').getAttribute('data-state');
  ...
  test.describe('Interruption Handling', () => {
    test('should handle user interruption gracefully', async ({ page }) => {
      ...
      await page.fill('[data-testid="chat-input"]', 'Tell me a very long story about space exploration');
      await page.click('[data-testid="send-button"]');
      ...
      await expect(page.locator('[data-testid="interruption-response"]')).toBeVisible({ timeout: 3000 });

      // Should offer continuation options
      await expect(page.locator('[data-testid="continuation-options"]')).toBeVisible({ timeout: 2000 });
    });
    ...
    test('should detect language in interruption', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="language-detected"]')).toContainText('es', { timeout: 3000 });
    });
    ...
    test('should offer continuation choices', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="speaking-indicator"]')).toBeVisible({ timeout: 3000 });
    });
  });
  ...
  test.describe('Accessibility', () => {
    test('should be accessible with keyboard navigation', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="speaking-indicator"]')).toBeVisible({ timeout: 5000 });
    });
    ...
    test('should provide screen reader announcements', async ({ page }) => {
      ...
      await expect(page.locator('[aria-live="polite"]')).toContainText('Speaking', { timeout: 5000 });
    });
  });
});
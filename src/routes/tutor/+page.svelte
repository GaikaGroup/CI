<script>
  ...
  $: authoredCourses = $user && $coursesStore.courses ? $coursesStore.courses.filter(course => 
    course.creatorId === $user.id || 
    (course.creatorRole === 'user' && !course.creatorId) // Backward compatibility
  ) : [];

  // Get students enrolled in tutor's courses
  $: enrolledStudents = $activeEnrollments
    .filter(enrollment => 
      authoredCourses.some(course => course.id === enrollment.courseId || course.id === enrollment.subjectId)
    )
    .map(enrollment => ({
      ...enrollment,
      course: authoredCourses.find(course => course.id === enrollment.courseId || course.id === enrollment.subjectId)
    }));
  ...
  const handleEditCourse = (event) => {
    ...
    const slug = course.name 
      ? course.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')
      : course.id;
  ...
  function getCourseEnrollmentCount(courseId) {
    return enrolledStudents.filter(student => 
      student.courseId === courseId || student.subjectId === courseId
    ).length;
  }
</script>
  ...
  <div class="mx-auto max-w-7xl px-4 py-8">
    ...
    <div class="mb-8">
      <div class="flex items-center gap-3 mb-2">
        <div class="flex items-center justify-center w-12 h-12 bg-amber-100 dark:bg-amber-900/40 rounded-xl">
    ...
    <div class="mb-8">
      <nav class="flex space-x-8 border-b border-stone-200 dark:border-gray-700">
        ...
        <button
          ...
          class="py-4 px-1 border-b-2 font-medium text-sm transition-colors {activeTab === 'analytics'
            ? 'border-amber-500 text-amber-600 dark:text-amber-400'
            : 'border-transparent text-stone-500 hover:text-stone-700 hover:border-stone-300 dark:text-gray-400 dark:hover:text-gray-300'}"
        >
          <div class="flex items-center gap-2">
    ...
    <div class="tab-content">
          ...
          {:else}
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
              <div class="bg-white dark:bg-gray-800 rounded-lg p-6 border border-stone-200 dark:border-gray-700">
                <div class="flex items-center justify-between">
                  <div>
                    <p class="text-sm font-medium text-stone-600 dark:text-gray-400">Total Courses</p>
              ...
              <div class="bg-white dark:bg-gray-800 rounded-lg p-6 border border-stone-200 dark:border-gray-700">
                <div class="flex items-center justify-between">
                  <div>
                    <p class="text-sm font-medium text-stone-600 dark:text-gray-400">Total Students</p>
              ...
              <div class="bg-white dark:bg-gray-800 rounded-lg p-6 border border-stone-200 dark:border-gray-700">
                <div class="flex items-center justify-between">
                  <div>
                    <p class="text-sm font-medium text-stone-600 dark:text-gray-400">Est. Lessons</p>
            ...
            <div class="grid gap-6 md:grid-cols-2">
              {#each authoredCourses as course}
                <article class="bg-white dark:bg-gray-800 rounded-2xl border border-stone-200 dark:border-gray-700 p-6 shadow-sm hover:shadow-md transition-shadow">
                  <div class="space-y-4">
                    <div class="flex items-start justify-between">
                      <div class="flex-1">
                        <div class="flex items-center gap-2 mb-1">
                          ...
                          <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {course.visibility === 'published' 
                    ...
                    {#if course.skills?.length}
                      <div class="flex flex-wrap gap-1">
                        {#each course.skills.slice(0, 3) as skill}
                          <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-stone-100 text-stone-700 dark:bg-gray-700 dark:text-gray-300">
                            {skill}
                          </span>
                        {/each}
                    ...
                    <!-- Course Stats -->
                    <div class="flex items-center justify-between text-sm text-stone-600 dark:text-gray-400">
      ...
      {:else if activeTab === 'create'}
        <!-- Create Course Tab -->
        <div class="space-y-6">
          <div class="bg-white dark:bg-gray-800 rounded-lg p-8 border border-stone-200 dark:border-gray-700 text-center">
            ...
            <p class="text-stone-600 dark:text-gray-400 mb-6 max-w-2xl mx-auto">
              Share your expertise with learners around the world. Our AI-powered course creation tools 
              will help you build engaging, structured learning experiences.
            </p>
            <button
              on:click={handleCreateCourse}
      ...
      {:else if activeTab === 'analytics'}
        ...
        <div class="space-y-6">
          <div class="bg-white dark:bg-gray-800 rounded-lg p-6 border border-stone-200 dark:border-gray-700">
            <h2 class="text-xl font-semibold text-stone-900 dark:text-white mb-6">Course Analytics</h2>
            ...
            {#if authoredCourses.length === 0}
              <div class="text-center py-8">
                <p class="text-stone-600 dark:text-gray-400">Create your first course to see analytics data.</p>
              </div>
            {/if}
          </div>
        </div>
          ...
          {:else}
            <div class="bg-white dark:bg-gray-800 rounded-lg border border-stone-200 dark:border-gray-700 overflow-hidden">
              <div class="px-6 py-4 border-b border-stone-200 dark:border-gray-700">
                <h2 class="text-xl font-semibold text-stone-900 dark:text-white">Enrolled Students</h2>
              ...
              <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-stone-200 dark:divide-gray-700">
                  <thead class="bg-stone-50 dark:bg-gray-900">
                    <tr>
                      <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-stone-500 dark:text-gray-400">
                        Student
                      </th>
                      <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-stone-500 dark:text-gray-400">
                        Course
                      ...
                      <th class="px-6 py-3 text-right text-xs font-medium uppercase tracking-wider text-stone-500 dark:text-gray-400">
                        Progress
                      </th>
                    </tr>
                  </thead>

  ...
  >
    ...
    >
      {#if title || showCloseButton}
        <div class="flex justify-between items-center p-4 border-b dark:border-gray-700 border-stone-200">
          {#if title}
            <h2 id="modal-title" class="text-lg font-semibold dark:text-white text-stone-900">{title}</h2>
          {:else}
            <div></div>
          {/if}
      ...
      {#if $$slots.footer}
        <div class="p-4 border-t dark:border-gray-700 border-stone-200 bg-stone-50 dark:bg-gray-750 rounded-b-lg">
          <slot name="footer" />
        </div>
      {/if}
    </div>

...
export async function loadOpenCV(): Promise<typeof cv> {
  return new Promise((resolve, reject) => {
    if (typeof window === 'undefined') return reject(new Error('OpenCV requires browser environment'));
...
async function imageToMat(cv: typeof window.cv, src: File | Blob | HTMLImageElement | string) {
  ...
  if (src instanceof HTMLImageElement) {
    ...
    await new Promise((res, rej) => { imgEl.onload = () => res(true); imgEl.onerror = rej; imgEl.src = url; });
...
export function globalDeskew(cv: typeof window.cv, src: any) {
  ...
  const bin = new cv.Mat(); cv.adaptiveThreshold(gray, bin, 255, cv.ADAPTIVE_THRESH_MEAN_C, cv.THRESH_BINARY, 31, 10);
  const edges = new cv.Mat(); cv.Canny(bin, edges, 60, 120);
  const lines = new cv.Mat(); cv.HoughLines(edges, lines, 1, Math.PI/180, 120);
  const angles: number[] = [];
  ...
  gray.delete(); bin.delete(); edges.delete(); lines.delete(); M.delete();
...
export function suppressGraphics(cv: typeof window.cv, grayOrBin: any) {
  ...
  const edges = new cv.Mat(); cv.Canny(work, edges, 80, 160);
  const lines = new cv.Mat(); cv.HoughLinesP(edges, lines, 1, Math.PI/180, 100, 80, 10);
  const mask = new cv.Mat.zeros(work.rows, work.cols, cv.CV_8UC1);
  for (let i=0; i<lines.rows; i++) {
    const arr = lines.intPtr(i) as unknown as number[];
    const x1 = arr[0], y1 = arr[1], x2 = arr[2], y2 = arr[3];
  ...
  work.delete(); edges.delete(); lines.delete(); mask.delete();
...
export function segmentTextBlocks(cv: typeof window.cv, srcGray: any) {
  ...
  for (let i=0; i<bboxes.rows; i++) {
    const x = bboxes.intAt(i,0), y = bboxes.intAt(i,1), w = bboxes.intAt(i,2), h = bboxes.intAt(i,3);
  ...
  for (const r of rects) {
    ...
    for (let i=0; i<merged.length; i++) {
      const m = merged[i];
      const rx = Math.max(r.x, m.x), ry = Math.max(r.y, m.y);
      const rX = Math.min(r.x+r.width, m.x+m.width), rY = Math.min(r.y+r.height, m.y+m.height);
  ...
  regions.delete(); bboxes.delete(); mser.delete();
  return merged;
}
...
export function preprocessBlock(cv: typeof window.cv, src: any, cfg: OCRConfig) {
  const gray = new cv.Mat(); cv.cvtColor(src, gray, cv.COLOR_RGBA2GRAY);
  const up = new cv.Mat(); cv.resize(gray, up, new cv.Size(0,0), cfg.upscale, cfg.upscale, cv.INTER_CUBIC);
  const den = new cv.Mat(); cv.bilateralFilter(up, den, 7, 50, 50);
  const bin = aThreshold(cv, den, cfg);
  gray.delete(); up.delete(); den.delete();
  return bin;
}

function aThreshold(cv: typeof window.cv, m: any, cfg: OCRConfig) {
  const bin = new cv.Mat();
  cv.adaptiveThreshold(m, bin, 255,
...
async function multipassOCR(worker: any, canvas: HTMLCanvasElement) {
  ...
  for (const pass of passes) {
    ...
    const conf = Array.isArray(data.symbols) && data.symbols.length
      ? data.symbols.reduce((a: number, s: any) => a + (s.confidence || 0), 0) / data.symbols.length
      : data.confidence ?? 0;
...
// ---------- Preview overlay ----------
export function drawROIsOnCanvas(srcCanvas: HTMLCanvasElement, items: Array<{ bbox: any; label: string; conf: number }>) {
  const canvas = document.createElement('canvas');
  canvas.width = srcCanvas.width; canvas.height = srcCanvas.height;
  const ctx = canvas.getContext('2d')!;
  ctx.drawImage(srcCanvas, 0, 0);
  ctx.lineWidth = 2; ctx.font = '14px ui-sans-serif'; ctx.textBaseline = 'top';
  for (const it of items) {
    const { x,y,width,height } = it.bbox as any;
    ctx.strokeStyle = '#22c55e'; ctx.strokeRect(x, y, width, height);
    ctx.fillStyle = 'rgba(34,197,94,0.15)'; ctx.fillRect(x, y, width, 18);
    ctx.fillStyle = '#111827'; ctx.fillText(`${it.label} ${Math.round(it.conf)}%`, x+4, y+2);
  }
  return canvas;
}
...
export async function ocrTextbookUniversal(
  ...
  try {
    ...
    const gray = new cv.Mat(); cv.cvtColor(deskewed, gray, cv.COLOR_RGBA2GRAY);
    ...
    for (const rect of blocks) {
      ...
      roiCanvas.width = bin.cols; roiCanvas.height = bin.rows;
      ...
      sub.delete(); bin.delete();
    }
    
    const preview = drawROIsOnCanvas(canvas, results.map(r=>({ bbox: r.bbox as any, label: 'text', conf: r.conf })));
    
    mat.delete(); deskewed.delete(); gray.delete(); cleaned.delete();

...
function playNextInQueue() {
  ...
  audioPlayer.onplay = () => {
    ...
    if (audioContext && audioAnalyser && audioPlayer) {
      try {
        // createMediaElementSource can only be invoked once per element
        if (!audioPlayer._sourceNode) {
          audioPlayer._sourceNode = audioContext.createMediaElementSource(audioPlayer);

          if (audioPlayer.gainNode) {
            audioPlayer._sourceNode.connect(audioPlayer.gainNode);
            audioPlayer.gainNode.connect(audioAnalyser);
          } else {
            audioPlayer._sourceNode.connect(audioAnalyser);
          }
          audioAnalyser.connect(audioContext.destination);
          console.log(
            'Connected audio player to analyzer',
            audioPlayer.gainNode ? 'with gain node for transitions' : ''
          );
        }
        ...
        console.log('Audio player connection error:', error.message);
        analyzeAudio();
      }
    }
  };
    ...
    if (phraseQueue.length > 0) {
      ...
      setTimeout(() => {
        if (audioPlayer.paused && phraseQueue.length === 0) {
          ...
          audioAmplitude.set(0);
          isPlayingSequence = false;
          console.log('Audio playback sequence completed');
        }
      }, 300);
    }

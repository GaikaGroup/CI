  ...
  try {
    ...
    if (updatedState) {
      console.log('Language state updated:', {
        ...
        previousLanguage:
          updatedState.confidenceHistory[updatedState.confidenceHistory.length - 2]?.language
      });
    } else {
...
export function exampleEnhanceChatMessages(sessionId, originalMessages) {
  ...
  try {
    ...
    console.log(
      `Enhanced messages: ${enhancedMessages.length} total (${enhancedMessages.length - originalMessages.length} added)`
    );
...
export async function exampleCompleteChatFlow(sessionId, userId, userMessage) {
  ...
  try {
    ...
    const simulatedResponse =
      userMessage.includes('русский') || userMessage.includes('Привет')
  ...
  for (const [scenarioName, scenario] of Object.entries(exampleScenarios)) {
    ...
    try {
      for (let i = 0; i < scenario.messages.length; i++) {
        ...
        const result = await exampleCompleteChatFlow(scenario.sessionId, scenario.userId, message);

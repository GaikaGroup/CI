 ...
 * @returns {Object[]} Filtered reports
 */
export function getReportsForCourse(reports, courseId) {
  if (!Array.isArray(reports)) {
    return [];
  }

  return reports.filter((report) => report.courseId === courseId);
}
...
/**
 * Check if user has already reported a course
 * @param {Object[]} reports - Array of reports
 * @param {string} courseId - Course ID
 * @param {string} reporterId - Reporter ID
 * @returns {boolean} True if user has already reported this course
 */
export function hasUserReportedCourse(reports, courseId, reporterId) {
  if (!Array.isArray(reports)) {
    return false;
  }

  return reports.some((report) => report.courseId === courseId && report.reporterId === reporterId);
}
 ...
 * @param {Object[]} reports - Array of reports
 * @param {string} courseId - Course ID
 * @returns {number} Number of reports for the course
 */
export function getReportCountForCourse(reports, courseId) {
  if (!Array.isArray(reports)) {
    return 0;
  }

  return reports.filter((report) => report.courseId === courseId).length;
}

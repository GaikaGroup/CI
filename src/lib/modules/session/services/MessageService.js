...
function validateMessageData(messageData) {
  ...
  if (content.length > 50000) {
    // 50KB limit for message content
...
function validateMetadata(metadata) {
  ...
  if (
    metadata.language &&
    (typeof metadata.language !== 'string' || metadata.language.length > 10)
  ) {
    throw new MessageValidationError(
  ...
  if (
    ...
    throw new MessageValidationError(
      'Timestamp must be a Date object or ISO string',
      'metadata.timestamp'
    );
  }
}
...
export class MessageService {
  ...
  static async addMessage(sessionId, type, content, metadata = null, userId = null) {
    ...
    return withRetry(async () => {
      try {
        ...
        throw new MessageError(`Failed to add message: ${error.message}`, 'MESSAGE_CREATE_FAILED', {
          sessionId,
          type,
  ...
  static async getSessionMessages(sessionId, options = {}, userId = null) {
    ...
    const { page = 1, limit = 50, sortOrder = 'asc', type = null } = options;
  ...
  static async getMessage(messageId, userId = null) {
    ...
    return withRetry(async () => {
      try {
        ...
        throw new MessageError(`Failed to get message: ${error.message}`, 'MESSAGE_FETCH_FAILED', {
          messageId,
          userId
        });
      }
  ...
  static async searchMessages(userId, query, options = {}) {
    ...
    const { page = 1, limit = 20, sessionId = null, type = null } = options;
  ...
  static async bulkDeleteMessages(sessionId, messageIds, userId = null) {
    ...
    return withRetry(async () => {
      try {
        ...
        console.log(
          `Bulk deleted ${result} messages from session: ${sessionId} by user: ${userId || 'system'}`
        );
        return result;
      } catch (error) {
        if (error instanceof SessionNotFoundError) {

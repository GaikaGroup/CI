...
export async function sessionWorkflowExample() {
  try {
    ...
    await chatStore.addAssistantMessage('Great! Arrays are fundamental data structures in JavaScript. Let me explain...');
...
export async function sessionUtilitiesExample() {
  try {
    ...
    const session = await sessionUtils.createAndStartSession(
      'Quick Math Help',
      'fun',
      'en'
    );
    ...
    const anotherSession = await sessionStore.createSession(
      'History Discussion',
      'learn',
      'en'
    );

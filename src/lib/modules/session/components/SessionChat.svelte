<script>
  ...
  import { Send, Loader2, AlertCircle, User, Bot, Mic, MicOff, MessageCircle, Image as ImageIcon } from 'lucide-svelte';
...
<div class="flex flex-col h-full {$darkMode ? 'bg-gray-800' : 'bg-white'}">
    ...
    <div class="flex items-center justify-between p-4">
      ...
      {#if $messageStats.total > 0}
        <div class="text-xs {$darkMode ? 'text-gray-400' : 'text-stone-500'}">
          {$messageStats.total} {$messageStats.total === 1 ? 'message' : 'messages'}
        </div>
      {/if}
    </div>
    ...
    <div class="flex justify-center pb-3">
      <div class="{$darkMode ? 'bg-gray-700' : 'bg-stone-100'} rounded-lg p-1 inline-flex">
        <button
          ...
          class="px-4 py-2 rounded-md font-medium text-sm transition-all flex items-center gap-2 {chatMode === 'text'
        ...
        <button
          ...
          class="px-4 py-2 rounded-md font-medium text-sm transition-all flex items-center gap-2 {chatMode === 'voice'
            ? 'bg-amber-600 text-white shadow-sm'
            : $darkMode
              ? 'text-gray-300 hover:text-amber-400'
              : 'text-stone-600 hover:text-amber-700'}"
  ...
  {#if chatMode === 'voice'}
    ...
    {#if $selectedImages.length > 0}
      <div class="p-4 flex flex-wrap gap-2 border-b {$darkMode ? 'border-gray-700' : 'border-stone-200'}">
      ...
      {#each $chatStore.messages as message (message.id)}
        <div class={getMessageClasses(message.type)}>
          ...
          <div class="flex-shrink-0">
            ...
            {:else}
              <div class="w-8 h-8 rounded-full {$darkMode ? 'bg-gray-600' : 'bg-stone-300'} flex items-center justify-center">
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="text-sm {$darkMode ? 'text-gray-300' : 'text-stone-700'} whitespace-pre-wrap break-words">
              {message.content}
            </div>
            ...
            {#if message.metadata}
              <div class="mt-2 space-y-2">
                ...
                {#if hasAudio(message)}
                  ...
                  <div class="flex items-center gap-2 p-2 rounded-lg {$darkMode ? 'bg-gray-800' : 'bg-stone-100'}">
                    ...
                    {#each imageUrls as imageUrl, index}
                      <div class="relative group">
                        <img 
                          ...
                          class="max-w-xs max-h-64 rounded-lg border {$darkMode ? 'border-gray-600' : 'border-stone-200'} object-cover cursor-pointer hover:opacity-90 transition-opacity"
                          on:click={() => {
                        ...
                        {#if message.metadata.images?.[index]?.dimensions}
                          <div class="absolute bottom-1 right-1 bg-black/50 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity">
                            {message.metadata.images[index].dimensions.width}×{message.metadata.images[index].dimensions.height}
                          </div>
                        {/if}
                      </div>
                    {/each}
                ...
                {#if message.metadata.type === 'voice' && message.type === 'user'}
                  <div class="flex items-center gap-1 text-xs {$darkMode ? 'text-gray-400' : 'text-stone-500'}">
                    <Mic class="w-3 h-3" />
  ...
  {:else}
    <!-- Voice Controls -->
    <div class="flex items-center justify-center space-x-4 p-6 border-t {$darkMode ? 'border-gray-700' : 'border-stone-200'}">
      <input
        type="file"
        bind:this={fileInput}
      ...
      <button
        ...
        class="p-3 transition-colors {$darkMode ? 'text-gray-400 hover:text-amber-400' : 'text-stone-500 hover:text-amber-600'}"

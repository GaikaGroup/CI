// Translations for the application
export const translations = {
  en: {
    title: 'mAItutors',
    signIn: 'Sign In',
    signOut: 'Sign Out',
    student: 'Student',
    tutor: 'Tutor',
    sessions: 'Sessions',
    sessionsLowercase: 'sessions',
    console: 'Console',
    admin: 'Admin',
    textChat: 'Text Chat',
    voiceChat: 'Voice Chat',
    voiceChatMode: 'Voice Chat Mode',
    talkToTutor: 'Talk to your AI tutor',
    recording: 'Recording...',
    processing: 'Processing...',
    holdToRecord: 'Hold to record',
    selectLanguage: 'Select Language',
    changeLanguage: 'Change Language',
    languageChanged: 'Language has been changed. Starting fresh conversation...',
    welcomeMessage: "Hello! I'm your AI tutor. How can I help you learn today?",
    placeholder: 'Type your message...',
    funModeNote: 'You are in Fun mode.',
    learnModeDefaultNote: 'You are in Learn mode.',
    learnModePracticeNote:
      "You are practicing {course} in practice mode. Let's focus on guided improvements.",
    learnModeExamNote:
      'You are preparing for {course} in exam mode. Expect stricter evaluation aligned with the official rubric.',
    learnSelectCourseHeading: 'Choose a course to study',
    learnSelectCourseDescription:
      'Pick a course and mode to tailor feedback to your exam preparation goals.',
    learnNoCourses: 'No courses available yet. Ask an admin to create one.',
    learnFocusSkills: 'Focus skills',
    learnPracticeSummary: 'Practice mode snapshot',
    learnExamSummary: 'Exam mode snapshot',
    learnPracticeMinWords: 'Suggested minimum length: {words} words',
    learnExamMinWords: 'Exam target length: {words} words',
    learnStartPractice: 'Start Practice Mode',
    learnStartExam: 'Start Exam Mode',
    learnCurrentCourse: 'Current course',
    learnChangeCourse: 'Change course',
    learnModePracticeLabel: 'Practice mode',
    learnModeExamLabel: 'Exam mode',
    learnModeActiveSummary: 'Mode focus',
    learnModeActiveInstructions: 'Guidance',
    learnModeActiveFollowUp: 'Follow-up expectations',
    learnModeActiveWordGoal: 'Target length',
    learnModeActiveSkills: 'Skills in focus',
    learnNavigationCodesHeading: 'Universal navigation codes',
    // Common actions
    create: 'Create',
    edit: 'Edit',
    delete: 'Delete',
    save: 'Save',
    cancel: 'Cancel',
    submit: 'Submit',
    search: 'Search',
    filter: 'Filter',
    close: 'Close',
    back: 'Back',
    next: 'Next',
    previous: 'Previous',
    loading: 'Loading...',
    error: 'Error',
    success: 'Success',
    confirm: 'Confirm',
    yes: 'Yes',
    no: 'No',
    // Sessions
    newSession: 'New Session',
    searchSessions: 'Search sessions...',
    filterByDate: 'Filter by date',
    filterByLanguage: 'Filter by language',
    filterByCommand: 'Filter by command',
    noSessions: 'No sessions found',
    deleteSession: 'Delete session',
    confirmDelete: 'Are you sure you want to delete this?',
    sessionDeleted: 'Session deleted successfully',
    // Login/Signup
    email: 'Email',
    password: 'Password',
    confirmPassword: 'Confirm password',
    forgotPassword: 'Forgot password?',
    dontHaveAccount: "Don't have an account?",
    alreadyHaveAccount: 'Already have an account?',
    signUp: 'Sign Up',
    loginWelcome: 'Access your personalized learning dashboard',
    rememberMe: 'Remember me',
    orContinueWith: 'or continue with',
    createNewSession: 'Create new session',
    noSessionsFound: 'No sessions found',
    tryAdjustingFilters: 'Try adjusting your filters or search query.',
    clearFilters: 'Clear filters',
    // Courses
    myCourses: 'My Courses',
    createCourse: 'Create Course',
    editCourse: 'Edit Course',
    deleteCourse: 'Delete Course',
    courseName: 'Course Name',
    courseDescription: 'Description',
    courseLanguage: 'Language',
    courseLevel: 'Level',
    noCourses: 'No courses available',
    continueLearning: 'Continue Learning',
    viewProgress: 'View Progress',
    enrolledOn: 'Enrolled on',
    lastActivity: 'Last activity',
    progress: 'Progress',
    lessons: 'Lessons',
    assessments: 'Assessments',
    active: 'Active',
    completed: 'Completed',
    // Admin
    adminDashboard: 'Admin Dashboard',
    userManagement: 'User Management',
    feedbackReview: 'Feedback Review',
    sessionManagement: 'Session Management',
    statistics: 'Statistics',
    totalUsers: 'Total Users',
    activeSessions: 'Active Sessions',
    // Signup specific
    firstName: 'First Name',
    lastName: 'Last Name',
    createAccount: 'Create Account',
    agreeToTerms: 'I agree to the Terms of Service and Privacy Policy',
    // Session page specific
    mySessions: 'My Sessions',
    yourSessions: 'Your Sessions',
    manageConversations: 'Manage and continue your conversations',
    funChat: 'Fun Chat',
    learnMode: 'Learn Mode',
    creating: 'Creating...',
    newChat: 'New Chat',
    startNewChat: 'Start New Chat',
    retry: 'Retry',
    noMoreSessions: 'No more sessions to load',
    // Student page
    studentDashboard: 'Student Dashboard',
    discoverCourses: 'Discover courses, track your learning progress, and achieve your goals',
    browseCourses: 'Browse Courses',
    myLearning: 'My Learning',
    availableCourses: 'Available Courses',
    discoverAndEnroll: 'Discover and enroll in courses to start your learning journey',
    enroll: 'Enroll',
    noCoursesEnrolled: 'No courses enrolled yet',
    startLearningJourney: 'Start your learning journey by browsing our course catalog',
    learningProgress: 'Learning Progress',
    totalCourses: 'Total Courses',
    lessonsCompleted: 'Lessons Completed',
    assessmentsTaken: 'Assessments Taken',
    noProgressData: 'No learning progress data available yet.',
    // Course edit page
    createCourseTitle: 'Create Course',
    editCourseTitle: 'Edit Course',
    loadingCourse: 'Loading course...',
    backToCatalogue: 'Back to Catalogue',
    // Login page features
    welcomeBack: 'Welcome back to AI Tutor',
    continueJourney:
      'Continue your personalized learning journey with our advanced AI-powered tutoring platform.',
    personalizedPaths: 'Personalized learning paths',
    aiAssistance: '24/7 AI assistance',
    progressTracking: 'Progress tracking',
    interactiveExercises: 'Interactive exercises',
    // Signup page
    startJourney: 'Start your learning journey',
    joinLearners:
      'Join thousands of learners who are accelerating their education with our AI-powered tutoring platform.',
    // Common labels
    pleaseLogin: 'Please log in to view your courses.',
    // Additional translations
    personalizedLearning: 'Personalized Learning',
    aiAdaptsToYou: 'AI adapts to your learning style and pace for optimal results',
    progressTrackingBenefit: 'Progress Tracking',
    monitorImprovement: 'Monitor your improvement with detailed analytics and insights',
    instantSupport: 'Instant Support',
    get247Help: 'Get help 24/7 from our advanced AI tutoring system',
    provenResults: 'Proven Results',
    joinSuccessful: 'Join successful learners achieving their academic goals',
    getStarted: 'Get started with your free AI Tutor account',
    activeEnrollment: 'Active enrollment',
    courseCatalogue: 'Course Catalogue',
    statisticsTitle: 'Statistics',
    lastHour: 'Last hour',
    last24Hours: 'Last 24 hours',
    last7Days: 'Last 7 days',
    last30Days: 'Last 30 days',
    lastYear: 'Last year',
    loadingStatistics: 'Loading statistics...',
    updating: 'Updating...',
    userActivity: 'User Activity',
    popularCourses: 'Popular Courses',
    provider: 'Provider',
    cost: 'Cost',
    messages: 'Messages',
    avgPerMsg: 'Avg/Msg',
    languageUsage: 'Language Usage',
    attentionEconomy: 'Attention Economy',
    fun: 'Fun',
    learn: 'Learn',
    // PDF Viewer
    clickToView: 'Click to view',
    downloadPdf: 'Download',
    closePdf: 'Close',
    platformHealth: 'Platform Health',
    userFeedback: 'User Feedback',
    noActivityData: 'No activity data',
    totalSessions: 'Total Sessions',
    totalMessages: 'Total Messages',
    totalCost: 'Total Cost',
    perMessage: 'per message',
    activeUsers: 'active',
    enrollments: 'enrollments',
    autoRefresh: 'Auto-refresh',
    autoRefreshEvery30: 'Auto-refresh every 30 seconds',
    courseError: 'Course Error',
    errorStatus: 'Error',
    refresh: 'Refresh',
    goToCourses: 'Go to Courses',
    exploreCatalogue:
      'Explore our course catalogue to find learning opportunities that match your interests.',
    tutorDashboard: 'Tutor Dashboard',
    manageCourses: 'Manage your courses and track student progress',
    courseManagement: 'Course Management',
    studentProgress: 'Student Progress',
    analytics: 'Analytics',
    usersManagement: 'Users Management',
    feedbackManagement: 'Feedback Management',
    sessionsManagement: 'Sessions Management',
    analyticsManagement: 'Analytics',
    financeManagement: 'Finance',
    homeTitle: 'AI-Powered Learning Platform',
    homeSubtitle: 'Transform your learning experience with personalized AI tutoring',
    getStartedNow: 'Get Started Now',
    learnMore: 'Learn More',
    howItWorks: 'How It Works',
    features: 'Features',
    testimonials: 'Testimonials',
    viewAll: 'View All',
    seeMore: 'See More',
    seeLess: 'See Less',
    showMore: 'Show More',
    showLess: 'Show Less',
    readMore: 'Read More',
    readLess: 'Read Less',
    exact: 'Exact',
    approximate: 'Approximate',
    // Additional missing keys
    catAvatar: 'Cat avatar',
    catMouth: 'Cat mouth',
    enrolled: 'Enrolled',
    debugEnrollments: 'Debug Enrollments',
    currentUser: 'Current User',
    activeEnrollments: 'Active Enrollments',
    enrollmentStats: 'Enrollment Stats',
    localStorageData: 'LocalStorage Data',
    clearLocalStorage: 'Clear LocalStorage',
    reloadPage: 'Reload Page',
    noActiveEnrollments: 'No active enrollments found',
    noEnrollmentStats: 'No enrollment stats found',
    subjectName: 'Subject Name',
    subjectDescription: 'Description',
    subjectLanguage: 'Language',
    subjectLevel: 'Level',
    skills: 'Skills',
    addSkill: 'Add a skill...',
    aiAgents: 'AI Agents',
    addAgent: 'Add Agent',
    specialistAgents: 'Specialist Agents',
    orchestrationAgent: 'Orchestration Agent',
    referenceMaterials: 'Reference Materials',
    uploadMaterials: 'Upload Materials',
    availableToAllAgents: 'Available to all agents',
    availableToAgents: 'Available to {count} agents',
    aiProviderSettings: 'AI Provider Settings',
    allowOpenAI: 'Allow OpenAI',
    preferredProvider: 'Preferred Provider',
    localLLM: 'Local LLM (Ollama)',
    openAI: 'OpenAI',
    fallbackEnabled: 'Fallback Enabled',
    enterSubjectName: 'Enter subject name...',
    enterLanguage: 'e.g., Spanish, English...',
    describeSubject: 'Describe what this subject covers and what learners will gain...',
    enterLevel: 'e.g., Beginner, B1, Advanced...',
    editSubject: 'Edit Subject',
    createNewSubject: 'Create New Subject',
    status: 'Status',
    errorRate: 'Error Rate',
    avgResponseTime: 'Avg Response Time',
    averageRating: 'Average Rating',
    totalReviews: 'Total Reviews',
    viewAllFeedback: 'View all →',
    funVsLearnMode: 'Fun vs Learn Mode',
    // Second Opinion Feature
    secondOpinion: 'Second Opinion',
    getSecondOpinion: 'Get Second Opinion',
    secondOpinionFrom: 'Second opinion from',
    generatingSecondOpinion: 'Getting second opinion...',
    noAlternativeProviders: 'No alternative models available',
    secondOpinionError: 'Error getting second opinion',
    secondOpinionSuccess: 'Second opinion generated',
    selectProvider: 'Select Provider',
    automaticSelection: 'Automatic Selection',
    manualSelection: 'Manual Selection',
    providerModel: 'Provider / Model',
    divergenceDetected: 'Differences detected',
    divergenceLow: 'Minor differences',
    divergenceMedium: 'Moderate differences',
    divergenceHigh: 'Significant differences',
    keyDifferences: 'Key Differences',
    suggestedQuestions: 'Suggested Follow-up Questions',
    wasThisHelpful: 'Was this helpful?',
    helpful: 'Helpful',
    notHelpful: 'Not Helpful',
    thankYouFeedback: 'Thank you for your feedback!',
    opinionCount: '{count} opinion',
    opinionCountPlural: '{count} opinions',
    viewOpinions: 'View Opinions',
    hideOpinions: 'Hide Opinions',
    thinking: 'Thinking...'
  },
  ru: {
    title: 'mAItutors',
    signIn: 'Войти',
    signOut: 'Выйти',
    student: 'Студент',
    tutor: 'Преподаватель',
    sessions: 'Сессии',
    sessionsLowercase: 'сессий',
    console: 'Консоль',
    admin: 'Админ',
    textChat: 'Текстовый чат',
    voiceChat: 'Голосовой чат',
    voiceChatMode: 'Режим голосового чата',
    talkToTutor: 'Поговорите с вашим ИИ-преподавателем',
    recording: 'Запись...',
    processing: 'Обработка...',
    holdToRecord: 'Удерживайте для записи',
    selectLanguage: 'Выберите язык',
    changeLanguage: 'Изменить язык',
    languageChanged: 'Язык был изменён. Начинаем новый разговор...',
    welcomeMessage: 'Привет! Я ваш ИИ-преподаватель. Как я могу помочь вам в обучении сегодня?',
    placeholder: 'Введите ваше сообщение...',
    funModeNote: 'Вы находитесь в режиме Fun.',
    learnModeDefaultNote: 'Вы находитесь в учебном режиме.',
    learnModePracticeNote:
      'Вы практикуете {course} в тренировочном режиме. Сосредоточимся на поддержке и подсказках.',
    learnModeExamNote:
      'Вы готовитесь к экзамену {course} в режиме экзамена. Ожидайте более строгие критерии и оценивание по рубрикам.',
    learnSelectCourseHeading: 'Выберите курс',
    learnSelectCourseDescription:
      'Выберите курс и режим, чтобы адаптировать подсказки под подготовку к экзамену.',
    learnNoCourses: 'Список курсов пуст. Попросите администратора добавить их.',
    learnFocusSkills: 'Ключевые навыки',
    learnPracticeSummary: 'Кратко о тренировочном режиме',
    learnExamSummary: 'Кратко об экзаменационном режиме',
    learnPracticeMinWords: 'Рекомендуемый минимальный объём: {words} слов',
    learnExamMinWords: 'Целевой объём на экзамене: {words} слов',
    learnStartPractice: 'Начать тренировку',
    learnStartExam: 'Начать экзамен',
    learnCurrentCourse: 'Текущий курс',
    learnChangeCourse: 'Сменить курс',
    learnModePracticeLabel: 'Тренировочный режим',
    learnModeExamLabel: 'Экзаменационный режим',
    learnModeActiveSummary: 'Основной фокус',
    learnModeActiveInstructions: 'Инструкции',
    learnModeActiveFollowUp: 'Что ожидается далее',
    learnModeActiveWordGoal: 'Целевой объём',
    learnModeActiveSkills: 'Акцент на навыках',
    learnNavigationCodesHeading: 'Универсальные коды навигации',
    // Common actions
    create: 'Создать',
    edit: 'Редактировать',
    delete: 'Удалить',
    save: 'Сохранить',
    cancel: 'Отмена',
    submit: 'Отправить',
    search: 'Поиск',
    filter: 'Фильтр',
    close: 'Закрыть',
    back: 'Назад',
    next: 'Далее',
    previous: 'Предыдущий',
    loading: 'Загрузка...',
    error: 'Ошибка',
    success: 'Успешно',
    confirm: 'Подтвердить',
    yes: 'Да',
    no: 'Нет',
    // Sessions
    newSession: 'Новая сессия',
    searchSessions: 'Поиск сессий...',
    filterByDate: 'Фильтр по дате',
    filterByLanguage: 'Фильтр по языку',
    filterByCommand: 'Фильтр по команде',
    noSessions: 'Сессии не найдены',
    deleteSession: 'Удалить сессию',
    confirmDelete: 'Вы уверены, что хотите удалить это?',
    sessionDeleted: 'Сессия успешно удалена',
    // Login/Signup
    email: 'Электронная почта',
    password: 'Пароль',
    confirmPassword: 'Подтвердите пароль',
    forgotPassword: 'Забыли пароль?',
    dontHaveAccount: 'Нет аккаунта?',
    alreadyHaveAccount: 'Уже есть аккаунт?',
    signUp: 'Регистрация',
    loginWelcome: 'Доступ к вашей персональной панели обучения',
    rememberMe: 'Запомнить меня',
    orContinueWith: 'или продолжить с',
    createNewSession: 'Создать новую сессию',
    noSessionsFound: 'Сессии не найдены',
    tryAdjustingFilters: 'Попробуйте изменить фильтры или поисковый запрос.',
    clearFilters: 'Очистить фильтры',
    // Courses
    myCourses: 'Мои курсы',
    createCourse: 'Создать курс',
    editCourse: 'Редактировать курс',
    deleteCourse: 'Удалить курс',
    courseName: 'Название курса',
    courseDescription: 'Описание',
    courseLanguage: 'Язык',
    courseLevel: 'Уровень',
    noCourses: 'Нет доступных курсов',
    continueLearning: 'Продолжить обучение',
    viewProgress: 'Посмотреть прогресс',
    enrolledOn: 'Записан',
    lastActivity: 'Последняя активность',
    progress: 'Прогресс',
    lessons: 'Уроки',
    assessments: 'Оценки',
    active: 'Активный',
    completed: 'Завершено',
    // Admin
    adminDashboard: 'Панель администратора',
    userManagement: 'Управление пользователями',
    feedbackReview: 'Просмотр отзывов',
    sessionManagement: 'Управление сессиями',
    statistics: 'Статистика',
    totalUsers: 'Всего пользователей',
    activeSessions: 'Активные сессии',
    // Signup specific
    firstName: 'Имя',
    lastName: 'Фамилия',
    createAccount: 'Создать аккаунт',
    agreeToTerms: 'Я согласен с Условиями использования и Политикой конфиденциальности',
    // Session page specific
    mySessions: 'Мои сессии',
    yourSessions: 'Ваши сессии',
    manageConversations: 'Управляйте и продолжайте ваши разговоры',
    funChat: 'Обычный чат',
    learnMode: 'Режим обучения',
    creating: 'Создание...',
    newChat: 'Новый чат',
    startNewChat: 'Начать новый чат',
    retry: 'Повторить',
    noMoreSessions: 'Больше нет сессий для загрузки',
    // Student page
    studentDashboard: 'Панель студента',
    discoverCourses: 'Открывайте курсы, отслеживайте прогресс обучения и достигайте целей',
    browseCourses: 'Обзор курсов',
    myLearning: 'Мое обучение',
    availableCourses: 'Доступные курсы',
    discoverAndEnroll: 'Откройте и запишитесь на курсы, чтобы начать обучение',
    enroll: 'Записаться',
    noCoursesEnrolled: 'Вы еще не записаны на курсы',
    startLearningJourney: 'Начните свое обучение, просмотрев каталог курсов',
    learningProgress: 'Прогресс обучения',
    totalCourses: 'Всего курсов',
    lessonsCompleted: 'Уроков завершено',
    assessmentsTaken: 'Оценок получено',
    noProgressData: 'Данные о прогрессе обучения пока недоступны.',
    // Course edit page
    createCourseTitle: 'Создать курс',
    editCourseTitle: 'Редактировать курс',
    loadingCourse: 'Загрузка курса...',
    backToCatalogue: 'Вернуться в каталог',
    // Additional translations
    welcomeBack: 'Добро пожаловать в AI Tutor',
    continueJourney:
      'Продолжайте свой персонализированный путь обучения с нашей продвинутой платформой на базе ИИ.',
    personalizedPaths: 'Персонализированные пути обучения',
    aiAssistance: 'Помощь ИИ 24/7',
    progressTracking: 'Отслеживание прогресса',
    interactiveExercises: 'Интерактивные упражнения',
    startJourney: 'Начните свой путь обучения',
    joinLearners:
      'Присоединяйтесь к тысячам учащихся, которые ускоряют свое образование с помощью нашей платформы на базе ИИ.',
    personalizedLearning: 'Персонализированное обучение',
    aiAdaptsToYou: 'ИИ адаптируется к вашему стилю и темпу обучения для оптимальных результатов',
    progressTrackingBenefit: 'Отслеживание прогресса',
    monitorImprovement: 'Отслеживайте свои улучшения с помощью детальной аналитики и инсайтов',
    instantSupport: 'Мгновенная поддержка',
    get247Help: 'Получайте помощь 24/7 от нашей продвинутой системы ИИ-репетиторства',
    provenResults: 'Проверенные результаты',
    joinSuccessful: 'Присоединяйтесь к успешным учащимся, достигающим своих академических целей',
    getStarted: 'Начните с бесплатного аккаунта AI Tutor',
    pleaseLogin: 'Пожалуйста, войдите, чтобы просмотреть свои курсы.',
    activeEnrollment: 'Активная запись',
    courseCatalogue: 'Каталог курсов',
    statisticsTitle: 'Статистика',
    lastHour: 'Последний час',
    last24Hours: 'Последние 24 часа',
    last7Days: 'Последние 7 дней',
    last30Days: 'Последние 30 дней',
    lastYear: 'Последний год',
    loadingStatistics: 'Загрузка статистики...',
    updating: 'Обновление...',
    userActivity: 'Активность пользователей',
    popularCourses: 'Популярные курсы',
    provider: 'Провайдер',
    cost: 'Стоимость',
    messages: 'Сообщения',
    avgPerMsg: 'Средн/Сообщ',
    languageUsage: 'Использование языков',
    attentionEconomy: 'Экономика внимания',
    fun: 'Развлечение',
    learn: 'Обучение',
    // PDF Viewer
    clickToView: 'Нажмите для просмотра',
    downloadPdf: 'Скачать',
    closePdf: 'Закрыть',
    platformHealth: 'Здоровье платформы',
    userFeedback: 'Отзывы пользователей',
    noActivityData: 'Нет данных об активности',
    totalSessions: 'Всего сессий',
    totalMessages: 'Всего сообщений',
    totalCost: 'Общая стоимость',
    perMessage: 'за сообщение',
    activeUsers: 'активных',
    enrollments: 'записей',
    autoRefresh: 'Авто-обновление',
    autoRefreshEvery30: 'Авто-обновление каждые 30 секунд',
    courseError: 'Ошибка курса',
    errorStatus: 'Ошибка',
    refresh: 'Обновить',
    goToCourses: 'Перейти к курсам',
    exploreCatalogue:
      'Изучите наш каталог курсов, чтобы найти возможности обучения, соответствующие вашим интересам.',
    tutorDashboard: 'Панель преподавателя',
    manageCourses: 'Управляйте своими курсами и отслеживайте прогресс студентов',
    courseManagement: 'Управление курсами',
    studentProgress: 'Прогресс студентов',
    analytics: 'Аналитика',
    usersManagement: 'Управление пользователями',
    feedbackManagement: 'Управление отзывами',
    sessionsManagement: 'Управление сессиями',
    analyticsManagement: 'Аналитика',
    financeManagement: 'Финансы',
    homeTitle: 'Платформа обучения на базе ИИ',
    homeSubtitle: 'Трансформируйте свой опыт обучения с персонализированным ИИ-репетиторством',
    getStartedNow: 'Начать сейчас',
    learnMore: 'Узнать больше',
    howItWorks: 'Как это работает',
    features: 'Возможности',
    testimonials: 'Отзывы',
    viewAll: 'Посмотреть все',
    seeMore: 'Показать больше',
    seeLess: 'Показать меньше',
    showMore: 'Развернуть',
    showLess: 'Свернуть',
    readMore: 'Читать далее',
    readLess: 'Свернуть',
    exact: 'Точно',
    approximate: 'Приблизительно',
    // Additional missing keys
    catAvatar: 'Аватар кота',
    catMouth: 'Рот кота',
    enrolled: 'Записан',
    debugEnrollments: 'Отладка записей',
    currentUser: 'Текущий пользователь',
    activeEnrollments: 'Активные записи',
    enrollmentStats: 'Статистика записей',
    localStorageData: 'Данные LocalStorage',
    clearLocalStorage: 'Очистить LocalStorage',
    reloadPage: 'Перезагрузить страницу',
    noActiveEnrollments: 'Активные записи не найдены',
    noEnrollmentStats: 'Статистика записей не найдена',
    subjectName: 'Название предмета',
    subjectDescription: 'Описание',
    subjectLanguage: 'Язык',
    subjectLevel: 'Уровень',
    skills: 'Навыки',
    addSkill: 'Добавить навык...',
    aiAgents: 'ИИ-агенты',
    addAgent: 'Добавить агента',
    specialistAgents: 'Специализированные агенты',
    orchestrationAgent: 'Агент-оркестратор',
    referenceMaterials: 'Справочные материалы',
    uploadMaterials: 'Загрузить материалы',
    availableToAllAgents: 'Доступно всем агентам',
    availableToAgents: 'Доступно {count} агентам',
    aiProviderSettings: 'Настройки ИИ-провайдера',
    allowOpenAI: 'Разрешить OpenAI',
    preferredProvider: 'Предпочитаемый провайдер',
    localLLM: 'Локальный LLM (Ollama)',
    openAI: 'OpenAI',
    fallbackEnabled: 'Резервный вариант включен',
    enterSubjectName: 'Введите название предмета...',
    enterLanguage: 'например, Испанский, Английский...',
    describeSubject: 'Опишите, что охватывает этот предмет и что получат учащиеся...',
    enterLevel: 'например, Начинающий, B1, Продвинутый...',
    editSubject: 'Редактировать предмет',
    createNewSubject: 'Создать новый предмет',
    status: 'Статус',
    errorRate: 'Частота ошибок',
    avgResponseTime: 'Среднее время ответа',
    averageRating: 'Средний рейтинг',
    totalReviews: 'Всего отзывов',
    viewAllFeedback: 'Посмотреть все →',
    funVsLearnMode: 'Развлечение vs Обучение',
    // Second Opinion Feature
    secondOpinion: 'Второе мнение',
    getSecondOpinion: 'Получить второе мнение',
    secondOpinionFrom: 'Второе мнение от',
    generatingSecondOpinion: 'Получаю второе мнение...',
    noAlternativeProviders: 'Нет доступных альтернативных моделей',
    secondOpinionError: 'Ошибка при получении второго мнения',
    secondOpinionSuccess: 'Второе мнение получено',
    selectProvider: 'Выбрать провайдера',
    automaticSelection: 'Автоматический выбор',
    manualSelection: 'Ручной выбор',
    providerModel: 'Провайдер / Модель',
    divergenceDetected: 'Обнаружены различия',
    divergenceLow: 'Незначительные различия',
    divergenceMedium: 'Умеренные различия',
    divergenceHigh: 'Значительные различия',
    keyDifferences: 'Ключевые различия',
    suggestedQuestions: 'Предлагаемые вопросы',
    wasThisHelpful: 'Это было полезно?',
    helpful: 'Полезно',
    notHelpful: 'Не полезно',
    thankYouFeedback: 'Спасибо за ваш отзыв!',
    opinionCount: '{count} мнение',
    opinionCountPlural: '{count} мнений',
    viewOpinions: 'Показать мнения',
    hideOpinions: 'Скрыть мнения',
    thinking: 'Думаю...'
  },
  es: {
    title: 'mAItutors',
    signIn: 'Iniciar sesión',
    signOut: 'Cerrar sesión',
    student: 'Estudiante',
    tutor: 'Tutor',
    sessions: 'Sesiones',
    sessionsLowercase: 'sesiones',
    console: 'Consola',
    admin: 'Admin',
    textChat: 'Chat de texto',
    voiceChat: 'Chat de voz',
    voiceChatMode: 'Modo de chat de voz',
    talkToTutor: 'Habla con tu tutor de IA',
    recording: 'Grabando...',
    processing: 'Procesando...',
    holdToRecord: 'Mantén presionado para grabar',
    selectLanguage: 'Seleccionar idioma',
    changeLanguage: 'Cambiar idioma',
    languageChanged: 'El idioma ha sido cambiado. Iniciando conversación nueva...',
    welcomeMessage: '¡Hola! Soy tu tutor de IA. ¿Cómo puedo ayudarte a aprender hoy?',
    placeholder: 'Escribe tu mensaje...',
    funModeNote: 'Estás en modo Fun.',
    learnModeDefaultNote: 'Estás en modo de aprendizaje.',
    learnModePracticeNote:
      'Estás practicando {course} en modo práctica. Nos centraremos en mejoras guiadas y comentarios útiles.',
    learnModeExamNote:
      'Te estás preparando para {course} en modo examen. Espera una evaluación más estricta alineada con la rúbrica oficial.',
    learnSelectCourseHeading: 'Elige un curso',
    learnSelectCourseDescription:
      'Selecciona un curso y un modo para adaptar la tutoría a tus objetivos de examen.',
    learnNoCourses: 'Todavía no hay cursos disponibles. Pide a un administrador que cree uno.',
    learnFocusSkills: 'Habilidades clave',
    learnPracticeSummary: 'Resumen del modo práctica',
    learnExamSummary: 'Resumen del modo examen',
    learnPracticeMinWords: 'Longitud mínima sugerida: {words} palabras',
    learnExamMinWords: 'Objetivo del examen: {words} palabras',
    learnStartPractice: 'Comenzar modo práctica',
    learnStartExam: 'Comenzar modo examen',
    learnCurrentCourse: 'Curso actual',
    learnChangeCourse: 'Cambiar curso',
    learnModePracticeLabel: 'Modo práctica',
    learnModeExamLabel: 'Modo examen',
    learnModeActiveSummary: 'Enfoque del modo',
    learnModeActiveInstructions: 'Indicaciones',
    learnModeActiveFollowUp: 'Próximos pasos',
    learnModeActiveWordGoal: 'Objetivo de longitud',
    learnModeActiveSkills: 'Habilidades en foco',
    learnNavigationCodesHeading: 'Códigos rápidos universales',
    // Common actions
    create: 'Crear',
    edit: 'Editar',
    delete: 'Eliminar',
    save: 'Guardar',
    cancel: 'Cancelar',
    submit: 'Enviar',
    search: 'Buscar',
    filter: 'Filtrar',
    close: 'Cerrar',
    back: 'Atrás',
    next: 'Siguiente',
    previous: 'Anterior',
    loading: 'Cargando...',
    error: 'Error',
    success: 'Éxito',
    confirm: 'Confirmar',
    yes: 'Sí',
    no: 'No',
    // Sessions
    newSession: 'Nueva sesión',
    searchSessions: 'Buscar sesiones...',
    filterByDate: 'Filtrar por fecha',
    filterByLanguage: 'Filtrar por idioma',
    filterByCommand: 'Filtrar por comando',
    noSessions: 'No se encontraron sesiones',
    deleteSession: 'Eliminar sesión',
    confirmDelete: '¿Estás seguro de que quieres eliminar esto?',
    sessionDeleted: 'Sesión eliminada exitosamente',
    // Login/Signup
    email: 'Correo electrónico',
    password: 'Contraseña',
    confirmPassword: 'Confirmar contraseña',
    forgotPassword: '¿Olvidaste tu contraseña?',
    dontHaveAccount: '¿No tienes cuenta?',
    alreadyHaveAccount: '¿Ya tienes cuenta?',
    signUp: 'Registrarse',
    loginWelcome: 'Accede a tu panel de aprendizaje personalizado',
    rememberMe: 'Recuérdame',
    orContinueWith: 'o continuar con',
    createNewSession: 'Crear nueva sesión',
    noSessionsFound: 'No se encontraron sesiones',
    tryAdjustingFilters: 'Intenta ajustar tus filtros o consulta de búsqueda.',
    clearFilters: 'Limpiar filtros',
    // Courses
    myCourses: 'Mis cursos',
    createCourse: 'Crear curso',
    editCourse: 'Editar curso',
    deleteCourse: 'Eliminar curso',
    courseName: 'Nombre del curso',
    courseDescription: 'Descripción',
    courseLanguage: 'Idioma',
    courseLevel: 'Nivel',
    noCourses: 'No hay cursos disponibles',
    continueLearning: 'Continuar aprendiendo',
    viewProgress: 'Ver progreso',
    enrolledOn: 'Inscrito el',
    lastActivity: 'Última actividad',
    progress: 'Progreso',
    lessons: 'Lecciones',
    assessments: 'Evaluaciones',
    active: 'Activo',
    completed: 'Completado',
    // Admin
    adminDashboard: 'Panel de administración',
    userManagement: 'Gestión de usuarios',
    feedbackReview: 'Revisión de comentarios',
    sessionManagement: 'Gestión de sesiones',
    statistics: 'Estadísticas',
    totalUsers: 'Total de usuarios',
    activeSessions: 'Sesiones activas',
    // Signup specific
    firstName: 'Nombre',
    lastName: 'Apellido',
    createAccount: 'Crear cuenta',
    agreeToTerms: 'Acepto los Términos de servicio y la Política de privacidad',
    // Session page specific
    mySessions: 'Mis sesiones',
    yourSessions: 'Tus sesiones',
    manageConversations: 'Gestiona y continúa tus conversaciones',
    funChat: 'Chat divertido',
    learnMode: 'Modo de aprendizaje',
    creating: 'Creando...',
    newChat: 'Nuevo chat',
    startNewChat: 'Iniciar nuevo chat',
    retry: 'Reintentar',
    noMoreSessions: 'No hay más sesiones para cargar',
    // Student page
    studentDashboard: 'Panel del estudiante',
    discoverCourses: 'Descubre cursos, rastrea tu progreso de aprendizaje y alcanza tus metas',
    browseCourses: 'Explorar cursos',
    myLearning: 'Mi aprendizaje',
    availableCourses: 'Cursos disponibles',
    discoverAndEnroll: 'Descubre e inscríbete en cursos para comenzar tu viaje de aprendizaje',
    enroll: 'Inscribirse',
    noCoursesEnrolled: 'Aún no estás inscrito en ningún curso',
    startLearningJourney: 'Comienza tu viaje de aprendizaje explorando nuestro catálogo de cursos',
    learningProgress: 'Progreso de aprendizaje',
    totalCourses: 'Total de cursos',
    lessonsCompleted: 'Lecciones completadas',
    assessmentsTaken: 'Evaluaciones realizadas',
    noProgressData: 'Aún no hay datos de progreso de aprendizaje disponibles.',
    // Course edit page
    createCourseTitle: 'Crear curso',
    editCourseTitle: 'Editar curso',
    loadingCourse: 'Cargando curso...',
    backToCatalogue: 'Volver al catálogo',
    // Additional translations
    welcomeBack: 'Bienvenido de nuevo a AI Tutor',
    continueJourney:
      'Continúa tu viaje de aprendizaje personalizado con nuestra plataforma avanzada impulsada por IA.',
    personalizedPaths: 'Rutas de aprendizaje personalizadas',
    aiAssistance: 'Asistencia de IA 24/7',
    progressTracking: 'Seguimiento del progreso',
    interactiveExercises: 'Ejercicios interactivos',
    startJourney: 'Comienza tu viaje de aprendizaje',
    joinLearners:
      'Únete a miles de estudiantes que están acelerando su educación con nuestra plataforma impulsada por IA.',
    personalizedLearning: 'Aprendizaje personalizado',
    aiAdaptsToYou:
      'La IA se adapta a tu estilo y ritmo de aprendizaje para obtener resultados óptimos',
    progressTrackingBenefit: 'Seguimiento del progreso',
    monitorImprovement: 'Monitorea tu mejora con análisis detallados e información',
    instantSupport: 'Soporte instantáneo',
    get247Help: 'Obtén ayuda 24/7 de nuestro sistema avanzado de tutoría con IA',
    provenResults: 'Resultados probados',
    joinSuccessful: 'Únete a estudiantes exitosos que logran sus objetivos académicos',
    getStarted: 'Comienza con tu cuenta gratuita de AI Tutor',
    pleaseLogin: 'Por favor, inicia sesión para ver tus cursos.',
    activeEnrollment: 'Inscripción activa',
    courseCatalogue: 'Catálogo de cursos',
    statisticsTitle: 'Estadísticas',
    lastHour: 'Última hora',
    last24Hours: 'Últimas 24 horas',
    last7Days: 'Últimos 7 días',
    last30Days: 'Últimos 30 días',
    lastYear: 'Último año',
    loadingStatistics: 'Cargando estadísticas...',
    updating: 'Actualizando...',
    userActivity: 'Actividad de usuarios',
    popularCourses: 'Cursos populares',
    provider: 'Proveedor',
    cost: 'Costo',
    messages: 'Mensajes',
    avgPerMsg: 'Prom/Msj',
    languageUsage: 'Uso de idiomas',
    attentionEconomy: 'Economía de atención',
    fun: 'Diversión',
    learn: 'Aprender',
    // PDF Viewer
    clickToView: 'Haga clic para ver',
    downloadPdf: 'Descargar',
    closePdf: 'Cerrar',
    platformHealth: 'Salud de la plataforma',
    userFeedback: 'Comentarios de usuarios',
    noActivityData: 'Sin datos de actividad',
    totalSessions: 'Total de sesiones',
    totalMessages: 'Total de mensajes',
    totalCost: 'Costo total',
    perMessage: 'por mensaje',
    activeUsers: 'activos',
    enrollments: 'inscripciones',
    autoRefresh: 'Auto-actualizar',
    autoRefreshEvery30: 'Auto-actualizar cada 30 segundos',
    courseError: 'Error del curso',
    errorStatus: 'Error',
    refresh: 'Actualizar',
    goToCourses: 'Ir a cursos',
    exploreCatalogue:
      'Explora nuestro catálogo de cursos para encontrar oportunidades de aprendizaje que coincidan con tus intereses.',
    tutorDashboard: 'Panel del tutor',
    manageCourses: 'Gestiona tus cursos y rastrea el progreso de los estudiantes',
    courseManagement: 'Gestión de cursos',
    studentProgress: 'Progreso de estudiantes',
    analytics: 'Analítica',
    usersManagement: 'Gestión de usuarios',
    feedbackManagement: 'Gestión de comentarios',
    sessionsManagement: 'Gestión de sesiones',
    analyticsManagement: 'Analítica',
    financeManagement: 'Finanzas',
    homeTitle: 'Plataforma de aprendizaje impulsada por IA',
    homeSubtitle: 'Transforma tu experiencia de aprendizaje con tutoría personalizada de IA',
    getStartedNow: 'Comenzar ahora',
    learnMore: 'Saber más',
    howItWorks: 'Cómo funciona',
    features: 'Características',
    testimonials: 'Testimonios',
    viewAll: 'Ver todo',
    seeMore: 'Ver más',
    seeLess: 'Ver menos',
    showMore: 'Mostrar más',
    showLess: 'Mostrar menos',
    readMore: 'Leer más',
    readLess: 'Leer menos',
    exact: 'Exacto',
    approximate: 'Aproximado',
    // Additional missing keys
    catAvatar: 'Avatar de gato',
    catMouth: 'Boca de gato',
    enrolled: 'Inscrito',
    debugEnrollments: 'Depurar inscripciones',
    currentUser: 'Usuario actual',
    activeEnrollments: 'Inscripciones activas',
    enrollmentStats: 'Estadísticas de inscripciones',
    localStorageData: 'Datos de LocalStorage',
    clearLocalStorage: 'Limpiar LocalStorage',
    reloadPage: 'Recargar página',
    noActiveEnrollments: 'No se encontraron inscripciones activas',
    noEnrollmentStats: 'No se encontraron estadísticas de inscripciones',
    subjectName: 'Nombre del tema',
    subjectDescription: 'Descripción',
    subjectLanguage: 'Idioma',
    subjectLevel: 'Nivel',
    skills: 'Habilidades',
    addSkill: 'Agregar una habilidad...',
    aiAgents: 'Agentes de IA',
    addAgent: 'Agregar agente',
    specialistAgents: 'Agentes especialistas',
    orchestrationAgent: 'Agente de orquestación',
    referenceMaterials: 'Materiales de referencia',
    uploadMaterials: 'Subir materiales',
    availableToAllAgents: 'Disponible para todos los agentes',
    availableToAgents: 'Disponible para {count} agentes',
    aiProviderSettings: 'Configuración del proveedor de IA',
    allowOpenAI: 'Permitir OpenAI',
    preferredProvider: 'Proveedor preferido',
    localLLM: 'LLM local (Ollama)',
    openAI: 'OpenAI',
    fallbackEnabled: 'Respaldo habilitado',
    enterSubjectName: 'Ingrese el nombre del tema...',
    enterLanguage: 'ej., Español, Inglés...',
    describeSubject: 'Describa qué cubre este tema y qué obtendrán los estudiantes...',
    enterLevel: 'ej., Principiante, B1, Avanzado...',
    editSubject: 'Editar tema',
    createNewSubject: 'Crear nuevo tema',
    status: 'Estado',
    errorRate: 'Tasa de error',
    avgResponseTime: 'Tiempo de respuesta promedio',
    averageRating: 'Calificación promedio',
    totalReviews: 'Total de reseñas',
    viewAllFeedback: 'Ver todo →',
    funVsLearnMode: 'Diversión vs Aprendizaje',
    // Second Opinion Feature
    secondOpinion: 'Segunda opinión',
    getSecondOpinion: 'Obtener segunda opinión',
    secondOpinionFrom: 'Segunda opinión de',
    generatingSecondOpinion: 'Obteniendo segunda opinión...',
    noAlternativeProviders: 'No hay modelos alternativos disponibles',
    secondOpinionError: 'Error al obtener segunda opinión',
    secondOpinionSuccess: 'Segunda opinión generada',
    selectProvider: 'Seleccionar proveedor',
    automaticSelection: 'Selección automática',
    manualSelection: 'Selección manual',
    providerModel: 'Proveedor / Modelo',
    divergenceDetected: 'Diferencias detectadas',
    divergenceLow: 'Diferencias menores',
    divergenceMedium: 'Diferencias moderadas',
    divergenceHigh: 'Diferencias significativas',
    keyDifferences: 'Diferencias clave',
    suggestedQuestions: 'Preguntas sugeridas',
    wasThisHelpful: '¿Fue esto útil?',
    helpful: 'Útil',
    notHelpful: 'No útil',
    thankYouFeedback: '¡Gracias por tu comentario!',
    opinionCount: '{count} opinión',
    opinionCountPlural: '{count} opiniones',
    viewOpinions: 'Ver opiniones',
    hideOpinions: 'Ocultar opiniones',
    thinking: 'Pensando...'
  }
};

// Helper function to get translations for the current language
export function getTranslation(lang, key) {
  if (!translations[lang]) {
    return translations.en[key] || key;
  }
  return translations[lang][key] || translations.en[key] || key;
}

 ...
 * for subject material processing. It maintains compatibility with existing
...
export class DocumentGraphRAGProcessor extends DocumentProcessor {
  ...
  /**
   * Process multiple documents for a subject
  ...
  async processMultipleDocuments(files, options = {}) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const fileOptions = {
        ...options,
        materialId: options.materialId || `${options.subjectId}_material_${i}`
      };
   ...
   */
  async queryGraphRAG(query, subjectId, agentIds = []) {
    try {
      return await this.graphRAGService.queryKnowledge(query, subjectId, agentIds);
    } catch (error) {
  ...
  /**
   * Create knowledge graph for a subject
  ...
  getKnowledgeGraph(subjectId) {
    return this.graphRAGService.getKnowledgeGraph(subjectId);
  }
   ...
   */
  getProcessingStats(subjectId) {
    const knowledgeGraph = this.getKnowledgeGraph(subjectId);

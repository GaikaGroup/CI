...
async function checkTestCoverage(testFilePath, endpointPath, method) {
  try {
    ...
    const hasEndpointReference =
      content.includes(normalizedEndpoint) || content.includes(endpointPath);
    ...
    const scenarios = {
      success: /20[01]|success|should\s+(?:create|return|get|update|delete).*successfully/i.test(
        content
      ),
...
function generateReport(results) {
  ...
  for (const result of results) {
    ...
    if (missing.length > 0) {
      console.log(
        '  ' + colors.red + 'Missing: ' + missing.map((s) => s.label).join(', ') + colors.reset
      );
    }
  ...
  const coveragePercentage =
    totalEndpoints > 0 ? Math.round((fullyCovered / totalEndpoints) * 100) : 0;
  ...
  if (coveragePercentage < 100) {
    console.log(
      colors.yellow + '  ⚠️  Not all endpoints have complete test coverage!' + colors.reset
    );
    console.log(
      colors.gray + '  Add tests for missing scenarios to improve coverage.\n' + colors.reset
    );
  } else {
    console.log(
      colors.green + '  🎉 All endpoints have complete test coverage!' + colors.reset + '\n'
    );
  }
}

...
### КРИТИЧЕСКОЕ ПРАВИЛО: Документация только после подтверждения

**НИКОГДА не создавай документацию пока не получишь подтверждение что все работает!**

#### Почему это важно:

1. **Расход токенов**: Создание документации расходует токены
2. **Двойная работа**: Если код не работает, документацию придется переделывать
3. **Ложная информация**: Документация о нерабочем коде вводит в заблуждение
4. **Потеря времени**: Пользователь и агент тратят время на переделку
...
6. ТОЛЬКО ТОГДА создавать документацию
```

#### Запрещенные действия:

- ❌ Создавать CHANGELOG.md до проверки работоспособности
- ❌ Создавать README.md с инструкциями до тестирования
- ❌ Создавать документацию по API до проверки endpoints
- ❌ Создавать отчеты о "успешной реализации" до физической проверки
- ❌ Обновлять документацию с новыми фичами до их тестирования

#### Разрешенные действия:

- ✅ Создавать TODO.md с планом работы (это не документация результата)
- ✅ Создавать NOTES.md с техническими заметками в процессе работы
- ✅ Обновлять tasks.md со статусом выполнения
- ✅ Создавать документацию ПОСЛЕ получения подтверждения от пользователя

#### Пример НЕПРАВИЛЬНОГО подхода:

```
Агент: "Создал новый API endpoint /api/courses"
Агент: "Создаю документацию..."
Агент: "Создал API_DOCUMENTATION.md с описанием endpoint"
Пользователь: "Проверил, endpoint возвращает 500 ошибку"
Агент: "Исправляю..."
[Теперь нужно переделывать и код, и документацию - двойная работа!]
```
...
```
Агент: "Создал новый API endpoint /api/courses"
Агент: "Проверяю работоспособность..."
Агент: "curl http://localhost:5173/api/courses - работает ✅"
Агент: "Запускаю тесты... ✅"
Агент: "Endpoint работает корректно. Создать документацию?"
Пользователь: "Да, все работает"
Агент: "Создаю API_DOCUMENTATION.md..."
```

#### Исключения:

Документацию можно создавать БЕЗ подтверждения только если:

1. Это обновление существующей документации с исправлением опечаток
2. Это документация процесса разработки (не результата)
3. Пользователь ЯВНО попросил создать документацию немедленно

#### Что делать если пользователь не отвечает:

```
Агент: "Код написан и проверен. Все тесты проходят ✅"
Агент: "Готов создать документацию. Подтвердите что все работает корректно?"
[Ждем ответа пользователя]
[НЕ создаем документацию автоматически!]
```
...
- ❌ **Создавать документацию до проверки и подтверждения пользователя**
- ❌ Говорить "Изменения применены" без подтверждения
- ❌ Предполагать что код работает
- ❌ Создавать CHANGELOG, README, API docs до физического тестирования

...
if (messagesWithImages.length > 0) {
  messagesWithImages.forEach((msg, idx) => {
    ...
    metadata.images.forEach((img, imgIdx) => {
      ...
      if (img.url) {
        ...
        console.log(`      Тип URL: ${isDataURL ? 'Data URL (base64)' : isHTTP ? 'HTTP URL' : 'Другое'}`);
  ...
  console.log(`   💾 Формат: ${messagesWithImages[0].metadata.images[0].url.startsWith('data:') ? 'Base64 Data URL' : 'HTTP URL'}`);
} else {
